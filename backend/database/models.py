...
class User(Base):
    ...
    system_states = relationship("SystemState", back_populates="user", cascade="all, delete-orphan")
...
class Persona(Base):
    ...
    # Relationships
    system_states = relationship("SystemState", back_populates="persona", cascade="all, delete-orphan")
...
class MemoryEntry(Base):
    ...
    meta_data = Column("metadata", JSON, nullable=True, default=dict)  # Use meta_data in Python, metadata in DB
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
    updated_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now(), onupdate=func.now())
